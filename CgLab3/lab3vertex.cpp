...
void Draw()
{
	...
	for (int i = 0; i < triangles.size(); ++i)
	{
		...
		vertices[0].reflectance = 100.0f;
		vertices[1].reflectance = 100.0f;
		vertices[2].reflectance = 100.0f;
...
void Interpolate(Pixel a, Pixel b, vector<Pixel> &result)
{
	...
	vec3 stepillumination = (b.illumination - a.illumination) / float(glm::max(N - 1, 1));
	...
	for (int j = 0; j < vertexPixels.size(); j++)
	{
		...
		for (Pixel l : line)
		{
			...
			if (l.x < leftPixels[row].x)
			{
				...
				leftPixels[row].illumination = l.illumination;
			}
			if (l.x >= rightPixels[row].x)
			{
				rightPixels[row].x = l.x;
				rightPixels[row].zinv = l.zinv;
				rightPixels[row].illumination = l.illumination;
			}
		}
	}
}
