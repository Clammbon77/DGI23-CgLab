...
void PixelShader(const Pixel &p)
{
	...
	if (p.zinv >= depthBuffer[x][y])
	{
		...
		vec3 light = currentReflectance * (lightPower * glm::max(glm::dot(currentNormal, glm::normalize(lightPos - p.pos3d)), 0.0f) / (4.0f * glm::pow(r, 2.0f) * pi) + indirectLightPowerPerArea);
